server <- function(input, output, session) {
  ...
  showNotification("Created by Imelda Sianturi", duration = NULL, type = "message")
  
# Use a reactive() function to prepare the base SQL query that all the elements in the dashboard
# will use. The reactive() tallows us to evaluate the input variables
...
# Total Flights (server) ------------------------------------------
  output$total_flights <- renderValueBox({
    ...
    base_flights() %>%
      ...
      as.integer() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("chart-pie"), color = "olive",subtitle = "Number of Flights")
  })
...
# Avg per Day (server) --------------------------------------------
  output$per_day <- renderValueBox({
    ...
    base_flights() %>%
      ...
      round() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("plane-departure"), color = "orange",subtitle = "Average Flights per day") 
  })
...
# Percent delayed (server) ----------------------------------------
  output$percent_delayed <- renderValueBox({
    base_flights() %>%
      ...
      round() %>%
      paste0("%") %>%
      valueBox(icon = icon("plane"), color = "lime", subtitle = "Flights delayed")
  })
